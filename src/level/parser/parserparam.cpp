...
CLevelParserParam::CLevelParserParam(unsigned char value)
  : m_value(boost::lexical_cast<std::string>(static_cast<unsigned int>(value)))
{}
...
CLevelParserParam::CLevelParserParam(EventType value)
  : m_value(FromEventType(value))
{}
...
glm::vec2 CLevelParserParam::AsPoint(glm::vec2 def)
{
    if (m_empty)
        return def;
    ...
    if (m_array.size() == 2)
    {
        return glm::vec2(m_array[0]->AsFloat(), m_array[1]->AsFloat());
    }
    else
    {
        throw CLevelParserExceptionBadParam(this, "point");
...
const std::string CLevelParserParam::FromEventType(EventType value)
{
    return boost::lexical_cast<std::string>(static_cast<int>(value));
}
