...
#define DO_WRITE_NEW(C)          auto line = std::make_unique<CLevelParserLine>(C);
#define DO_WRITE_VAL(K,V)        line->AddParam((K), std::make_unique<CLevelParserParam>(V));
...
#define DO_TST_UNMATCHED()       for(auto k:line->GetKeys()) if(!keys.count(k)) {ThrowUnusedArg(line,k,__FILE__,__LINE__);}
