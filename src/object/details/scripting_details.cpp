...
{
    name           = CScriptingHardcodeCollection::GetNameInScriptFiles(type);
    alias          = CScriptingHardcodeCollection::GetAliasInScriptFiles(type);
    helpTopicPath  = CScriptingHardcodeCollection::GetHelpTopicPathName(type);

    produce.isProducable      = CScriptingHardcodeCollection::IsValidObjectTypeId(type);
    produce.isProducedCharged = CScriptingHardcodeCollection::IsProduceAlreadyCharged(type);
    produce.isProducedManual  = CScriptingHardcodeCollection::IsProduceManual(type);
    produce.container         = CScriptingHardcodeCollection::GetProduceContainer(type);

    findableByRadar = CScriptingHardcodeCollection::GetObjectsFindableByType(type);
}
