...
{
    enabled       = CShieldedHardcodeCollection::IsShielded(type);
    nonRepairable = CShieldedHardcodeCollection::IsNonRepairable(type);
}
...
bool CObjectShieldedDetails::Read(CLevelParserLine* line)
{
    ...
    READ_ARG( "nonRepairable", AsBool, nonRepairable );
    READ_END();
...
void CObjectShieldedDetails::Write(CLevelParser* parser, ObjectType type)
{
    ...
    WRITE_ARG( "nonRepairable", AsBool, nonRepairable );
    WRITE_END();
}
