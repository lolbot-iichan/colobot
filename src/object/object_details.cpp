...
void CObjectDetails::LoadHardcode()
{
    for ( int i = 0 ; i < OBJECT_MAX ; i++ )
    {
        ...
        m_objects[type].automated.ReadHardcode(type);
        m_objects[type].detectable.ReadHardcode(type);
        m_objects[type].physical.ReadHardcode(type);
        m_objects[type].level.ReadHardcode(type);
        m_objects[type].scripting.ReadHardcode(type);
        m_objects[type].modeled.ReadHardcode(type);
        ...
        m_objects[type].exchange_post.ReadHardcode(type);
...
void CObjectDetails::Load(const char* fname)
{
    ...
    for (auto& line : parser.GetLines())
    {
        if ( line->GetCommand() == "SetObjectNaming" )
        {
            ...
            int base = line->GetParam("template")->AsInt(-1);
            if ( base != -1 )
                m_objects[type] = m_objects[static_cast<ObjectType>(base)];
        ...
        if ( line->GetParam("type")->IsDefined() )
        {
            ...
            if (m_objects[type].assistant.Read(line.get()) ||
                m_objects[type].automated.Read(line.get()) ||
                m_objects[type].detectable.Read(line.get()) ||
                m_objects[type].physical.Read(line.get()) ||
                m_objects[type].level.Read(line.get()) ||
                m_objects[type].scripting.Read(line.get()) ||
                m_objects[type].modeled.Read(line.get()) ||
                ...
                m_objects[type].shielder.Read(line.get()) ||
                m_objects[type].exchange_post.Read(line.get()) ) continue;
            else
                ThrowUnusedCmd(line.get());
        }

        if ( m_globals.Read(line.get()) ) continue;
        else ThrowUnusedCmd(line.get());
    }
}
...
void CObjectDetails::Dump(const char* fname)
{
    ...
    for (int i = 0; i < OBJECT_MAX; i++)
    {
        ...
        m_objects[type].automated.Write(&parser, type);
        m_objects[type].detectable.Write(&parser, type);
        m_objects[type].physical.Write(&parser, type);
        m_objects[type].level.Write(&parser, type);
        m_objects[type].scripting.Write(&parser, type);
        m_objects[type].modeled.Write(&parser, type);
        ...
        m_objects[type].exchange_post.Write(&parser, type);
    }
