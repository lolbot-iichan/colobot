...
Error CTaskDeflag::Start()
{
    ...
    auto task = GetObjectTaskExecutorDetails(m_object).deflag;
...
CObject* CTaskDeflag::SearchNearest(glm::vec3 pos)
{
...
{
    auto deflag = GetObjectTaskExecutorDetails(m_object).deflag;
    glm::mat4 mat = m_object->GetWorldMatrix(deflag.partNum);
    glm::vec3 iPos = Math::Transform(mat, deflag.position);

    CObject* pObj = SearchNearest(iPos);
    if ( pObj == nullptr || glm::distance(iPos, pObj->GetPosition()) > deflag.radius )
    {
