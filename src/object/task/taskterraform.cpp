...
bool CTaskTerraform::EventProcess(const Event &event)
{
    ...
    if ( m_phase == TTP_CHARGE )
    {
        ...
        if (CObject* power = GetObjectInPowerCellSlot(m_object))
        {
            power->SetScale(1.0f+m_progress*1.0f);
        }

        float energy = event.rTime * ENERGY_TERRA / 4.0f;
...
{
    auto task = GetObjectTaskExecutorDetails(m_object).thump;

    m_bError = true;  // operation impossible
    {
        float energy = ENERGY_TERRA;
        ...
        m_terraPos = Math::Transform(mat, task.position);
...
bool CTaskTerraform::Terraform()
{
    ...
    auto thump = GetObjectTaskExecutorDetails(m_object).thump;
    ...
    for (CObject* pObj : CObjectManager::GetInstancePointer()->GetAllObjects())
    {
        ...
        {
            if ( dist > thump.radius )  continue;
            if (!pObj->Implements(ObjectInterfaceType::Thumpable))  continue;

            for ( auto it : thump.objects )
            {
                if ( it.target != pObj->GetType() )  continue;
                ...
                motion->SetAction(it.action, duration);
